...
export default function ChooseGames() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const nextState = await refreshSession({ force: true });
      if (!nextState.hasGames) {
        setError('Não foi possível confirmar seus jogos agora. Tente novamente.');
        return;
      }
      if (nextState.hasInterests) {
        router.replace('/home');
      } else {
